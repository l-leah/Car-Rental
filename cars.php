				...
				<div class="card">
					<div class="card-header">
						...
						<span class="float:right"><a class="btn btn-primary btn-block btn-sm col-sm-2 float-right"
								href="index.php?page=manage_car" id="new_car">
					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								...
								$cars = $conn->query("SELECT * FROM cars order by brand asc,model asc, price asc, qty asc");
								while ($row = $cars->fetch_assoc()):
									...
									<tr>
										...
										<td>
											<p> <b>
													<?php echo ucwords($cat[$row['category_id']]) ?>
												</b></p>
										</td>
										<td>
											<p>Transmission: <b>
													<?php echo ucwords($tt[$row['transmission_id']]) ?>
												</b></p>
											<p>Engine: <b>
													<?php echo ucwords($et[$row['engine_id']]) ?>
												</b></p>
										</td>
										<td>
											<p> <b>
										...
										<td>
											<p> <b>
													<?php echo ucwords($row['qty']) ?>
												</b></p>
										</td>
										<td class="text-center">
											<button class="btn btn-sm btn-primary view_car" type="button"
												data-id="<?php echo $row['id'] ?>">View</button>
											<button class="btn btn-sm btn-primary edit_car" type="button"
												data-id="<?php echo $row['id'] ?>">Edit</button>
											<button class="btn btn-sm btn-danger delete_car" type="button"
